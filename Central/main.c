...
typedef struct
{
    ble_gap_addr_t         * p_addr; /**< List contains peripherals addreses. */
    ble_advdata_tk_value_t * p_tk;   /**< List contains peripherals tk values. */
    ble_lls_c_t            * p_lls_c;
    // bool											connected;
}clients_data;

static uint8_t               m_nbr_of_clients = 0; /**< Contains  nbr of actual client's in database. */
static ble_gap_scan_params_t m_scan_param;         /**< Scan parameters requested for scanning and connection. */
static clients_data          clients[MAX_PEER_COUNT];

static ble_gap_addr_t m_peer_addr_actual;
static uint8_t        m_peer_nbr_actual;

static uint8_t     p_tx_data[UART_DATA_LEN];
static uint8_t     p_rx_data[UART_DATA_LEN];
static ble_lls_c_t m_lls_clients[MAX_PEER_COUNT]; /**< Table contains structures used to identify lls_clients. */
...
static void java_client_remove(ble_gap_addr_t         * p_user_address,
                               ble_advdata_tk_value_t * p_user_password);

static void lls_timeout_handler(void * p_context)
{
    java_client_remove(clients[m_peer_nbr_actual].p_addr, clients[m_peer_nbr_actual].p_tk);
}
                                               ...
                                               const ret_code_t    event_result)
{
    ...
    switch (p_event->event_id)
    {
        ...
        case DM_EVT_SECURITY_SETUP_COMPLETE:
            APPL_LOG(
                "[APPL]:[0x%02X] >> DM_EVT_SECURITY_SETUP_COMPLETE, result 0x%08X. Conn id is %x\r\n",
                p_handle->connection_id,
                event_result,
                p_handle->connection_id);
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_ADV_REPORT:
            ...
            if (client_get(&m_peer_addr_actual, &m_peer_nbr_actual) != NRF_SUCCESS)
            {
        ...
        case BLE_GAP_EVT_AUTH_KEY_REQUEST:
            ...
            err_code = sd_ble_gap_auth_key_reply(p_ble_evt->evt.gap_evt.conn_handle,
                                                 ...
                                                 clients[m_peer_nbr_actual].p_tk->tk);
            APP_ERROR_CHECK(err_code);
...
static void lls_c_evt_handler(ble_lls_c_t * p_lls_c, ble_lls_c_evt_t * p_lls_c_evt)
{
    ...
    switch (p_lls_c_evt->evt_type)
    {
        case BLE_LLS_C_EVT_DISCOVERY_COMPLETE:
            ...
            APP_ERROR_CHECK(err_code);
            clients[m_peer_nbr_actual].p_lls_c = p_lls_c;


            uint32_t err_code = app_timer_create(&m_lls_timer[m_peer_nbr_actual],
                                                 APP_TIMER_MODE_SINGLE_SHOT,
                                                 lls_timeout_handler);
            APP_ERROR_CHECK(err_code);
            err_code = app_timer_start(m_lls_timer[m_peer_nbr_actual], LLS_INTERVAL, NULL);
            APP_ERROR_CHECK(err_code);
...
void lls_c_init()
{
    ...
    for (uint8_t i = 0; i < MAX_PEER_COUNT; i++)
    {
        p_lls_clients[i] = m_lls_clients + i;
    }
...
static uint32_t client_database_add_client(ble_gap_addr_t         * p_client_addr,
                                           ble_advdata_tk_value_t * p_client_tk)
{
    if (m_nbr_of_clients > MAX_PEER_COUNT - 1 )
    {
        APPL_LOG("[APPL]: Too many devices in whitelist. \r\n");
        return NRF_ERROR_DATA_SIZE;
    }

    clients[m_nbr_of_clients].p_addr = p_client_addr;
    clients[m_nbr_of_clients].p_tk   = p_client_tk;
// clients[m_nbr_of_clients].connected = false;

    APPL_LOG(
        "[APPL]: Add client with address: [%02X %02X %02X %02X %02X %02X]. \r\n whitelist actual nbr %x\r\n",
        p_client_addr->addr[0],
        p_client_addr->addr[1],
        p_client_addr->addr[2],
        p_client_addr->addr[3],
        p_client_addr->addr[4],
        p_client_addr->addr[5],
        m_nbr_of_clients);
...
static uint8_t client_get(ble_gap_addr_t * p_client_addr, uint8_t * p_client_nbr)
{
    for (uint8_t i = 0; i < m_nbr_of_clients; i++)
    {
        // if(memcmp(p_client_addr->addr, p_whitelist_addr[i]->addr, BLE_GAP_ADDR_LEN) ==0)
        if (memcmp(p_client_addr, clients[i].p_addr, BLE_GAP_ADDR_LEN) == 0) // consider that it compare whole address with address type.
        {
...
static uint32_t client_database_remove_client(ble_gap_addr_t * p_client_addr)
{
    ...
    {
        clients[client_nbr].p_addr = clients[client_nbr + 1].p_addr;
        clients[client_nbr].p_tk   = clients[client_nbr + 1].p_tk;
        // clients[client_nbr].connected  = clients[client_nbr+1].connected;
    }

    ++client_nbr;

    clients[client_nbr].p_addr = NULL; // the last client.
    clients[client_nbr].p_tk   = NULL;
    // clients[client_nbr].connected=false;
...
static void java_buffer_prepare(uint8_t const * const cmd,
                                ble_gap_addr_t const * const p_address,
                                ble_advdata_tk_value_t const * const p_password, uint8_t * p_data)
{
    uint8_t offset = 0;

    memcpy(p_data, cmd, CMD_LEN);
    offset += CMD_LEN;
    memcpy(p_data + offset, p_address->addr, BLE_GAP_ADDR_LEN);
    offset += BLE_GAP_ADDR_LEN;
    memcpy(p_data + offset, p_password->tk, BLE_GAP_SEC_KEY_LEN);
    offset += BLE_GAP_SEC_KEY_LEN;
...
static uint32_t java_cmd_send_rec(uint8_t const * const                cmd,
                                  ble_gap_addr_t const * const         p_address,
                                  ble_advdata_tk_value_t const * const p_password)
{
// uint32_t err_code;

    java_buffer_prepare(cmd, p_address, p_password, p_tx_data);

    /*	for(uint8_t i=0; i<UART_DATA_LEN; i++)
        {
                err_code=app_uart_put(p_tx_data[i]);
                if(err_code!=NRF_SUCCESS)
                {
                            return err_code;
                }
        ...
        while(app_uart_get(p_rx_data) != NRF_SUCCESS);

        if(memcmp("OK", p_rx_data, UART_DATA_LEN) ==0) // if we receive the same data-everything is ok.
        {
                return NRF_SUCCESS;
        }
...
static void java_client_add(ble_gap_addr_t const * const         p_user_address,
                            ble_advdata_tk_value_t const * const p_user_password)
{
    uint32_t err_code;
    uint8_t  cmd[CMD_LEN] = {'B', 'i', 'k', 'e', 'A', 'd', 'd'};


    err_code = java_cmd_send_rec(cmd, p_user_address, p_user_password);
    APP_ERROR_CHECK(err_code);

    err_code =
        client_database_add_client((ble_gap_addr_t *)p_user_address,
                                   ( ble_advdata_tk_value_t *) p_user_password);
    APP_ERROR_CHECK(err_code);
    APPL_LOG("[APPL]: Adding client to database");
                               ...
                               ble_advdata_tk_value_t * p_user_password)
{
    ...
    uint8_t cmd[CMD_LEN] = {'B', 'i', 'k', 'e', 'D', 'e', 'l'};

    err_code = java_cmd_send_rec(cmd, p_user_address, p_user_password);
    APP_ERROR_CHECK(err_code);

    err_code = client_get(p_user_address, &client_nbr);
    APP_ERROR_CHECK(err_code);
    err_code = ble_lls_c_alert_remove(clients[client_nbr].p_lls_c);
    APP_ERROR_CHECK(err_code);

    err_code = sd_ble_gap_disconnect(clients[client_nbr].p_lls_c->conn_handle,
                                     BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
    APP_ERROR_CHECK(err_code);

    err_code = client_database_remove_client(p_user_address);
    APP_ERROR_CHECK(err_code);

    APPL_LOG("[APPL]: Removing client from database");
}
...
int main(void)
{
    ...
    java_client_add(client_addr, &client_tk);
    java_client_add(&client_addr[1], &client_tk);
