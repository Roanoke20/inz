...
static void service_discover(client_t * p_client)
{
	...
	//here
	printf("err code= %d", err_code);
	printf("Number of conn is %d handle %d\r\n", p_client->srv_db.srv_count, p_client->srv_db.conn_handle);
...
{
		printf("Found type is %x", p_evt->evt_type );
        ...
        p_evt->params.discovered_db.srv_uuid.type == BLE_UUID_TYPE_BLE)
    {
        ...
        for (i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
            ...
            p_characteristic = &(p_evt->params.discovered_db.charateristics[i]);
					...
					printf("%x ", p_characteristic->characteristic.uuid.uuid);
...
uint32_t client_handling_create(const dm_handle_t * p_handle, uint16_t conn_handle)
{
	...
	//Service discovering.
    err_code = ble_db_discovery_start(&(m_client[p_handle->connection_id].srv_db),
                                      m_client[p_handle->connection_id].srv_db.conn_handle);
	//here
	printf("err code= %d", err_code);
	printf("Number of conn is %d handle %d\r\n", m_client[p_handle->connection_id].srv_db.srv_count, m_client[p_handle->connection_id].srv_db.conn_handle);
    APP_ERROR_CHECK(err_code);
